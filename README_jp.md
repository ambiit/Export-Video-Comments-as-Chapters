...
Eagleアプリで入力した動画コメントをチャプターファイルとして出力するプラグインです。  
mpvをベースとした動画プレーヤーが動画コメントをチャプターとして読み込んで再生できるようにします。
...
- Eagleアプリの動画コメントがmpvのチャプターとして読み込まれ、動画が再生されます

## インストールと必要な設定

- Eagleアプリの [プラグイン] メニューから [プラグインセンター] を選び、 [動画コメントをチャプターとして出力] をインストールします。

- mpvをベースとした動画プレーヤーをインストールします。
  - mpv (オリジナル): [https://mpv.io](https://mpv.io)
...
- mpvで外部のチャプターファイルを読み込むためのスクリプト`chapter-make-read.lua`をダウンロードします。(`chapter-make-read.lua`はサードパーティーによるmpv非公式のスクリプトです。)
